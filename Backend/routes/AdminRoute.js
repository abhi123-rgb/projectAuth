...
const { AdminsignupSchema, signinSchema } = require("../utils/validator");
const adminRoute = express.Router();

adminRoute.post("/adminsignup", validate(AdminsignupSchema), async (req, res) => {
  try {
    const { fullName, email, password, accessCode, role } = req.body;
    ...
    if (accessCode !== process.env.ACCESS_CODE) {
        return res.status(400).json({
            message: "Incorrect Admin access code! Unauthorize",
        });
    }
...
adminRoute.get("/getallAdminEmployee",authorize,RoleAuth("Admin"),async (req, res) => {
    ...
    const getadminemp = await Employee.find({
      role: { $in: ["Manager", "Employee"] },
      userId: user._id,
    })
      .populate("userId")
      .select("-password ");
    const filteredData = getadminemp.filter(
...
adminRoute.get("/getallAdmins",authorize,RoleAuth("Admin"),async (req, res) => {
  const { user } = req.user;
  ...
  if (!getadmin) return res.status(401);
